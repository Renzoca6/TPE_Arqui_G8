...
static int ai_cell_free(const TronGame *G, int c, int r) {
    return cell_is_free(G, c, r);
}

static int ai_move_towards(const TronGame *G, const Player *bot, int tc, int tr, player_Intent *out) {
    int bc = (int)bot->col, br = (int)bot->row;
    int dc = tc - bc,      dr = tr - br;
...
int ai_choose_dir_cutoff(const TronGame *G, const Player *bot, const Player *target, player_Intent *out) {
    ...
    int tdx = target->dx, tdy = target->dy;

    // Si está quieto, no conviene cortar → fallback
    if (tdx == 0 && tdy == 0) goto fallback;

    int fut_c[3], fut_r[3];
    fut_c[0] = (int)target->col + tdx; fut_r[0] = (int)target->row + tdy;
    fut_c[1] = fut_c[0] + tdx;         fut_r[1] = fut_r[0] + tdy;
    fut_c[2] = fut_c[1] + tdx;         fut_r[2] = fut_r[1] + tdy;

    // 2) Intentar moverse hacia cada futura celda
    for (int k = 0; k < 3; k++) {
        int tc = fut_c[k], tr = fut_r[k];
        if (!cell_is_inside(&G->grid, tc, tr)) continue;
        if (ai_move_towards(G, bot, tc, tr, out)) return 1;
    }

fallback:
    // 3) Si no pudo cortar, que lo persiga…
    if (ai_choose_dir_track(G, bot, target, out)) return 1;

    // 4) …y si tampoco, IA simple
    return ai_choose_dir_simple(G, bot, out);
}

...
// Frecuencia actual del timer son lso ticks por segundo
static uint32_t g_tick_hz = 0;
extern void outb(uint16_t port, uint8_t val);


// es llamada desde el interrupts dispatcher
...
// Devuelve la cantidad total desde que se inicio 
uint64_t timer_ticks(void) {
    return g_ticks;
}

// Devuelve el tiempo en milisegundos desde que se inicio
